...
begin
   --  Peras := 2;
   --  Manzanas := 4;
   --  -- Peras := Manzanas; -- En cualquier lenguaje esto es una asignación válida, pero ADA no
   --  Frutas := Peras + Manzanas; -- Esta es una asignación válida y semánticamente correcta
   --  -- Total := Peras + Manzanas; -- Esta no es una asignación válida
   --  Put_Line("En total tengo" & Frutas'Image & " frutas");
   --
   --  Frutas := 300;
   --  Frutas := Frutas + 50; -- Falla en tiempo de ejecución si --type Cantidad is range 0..300;
   ...
   --     Base := 10.0;
   --     Altura := 30.0;
   --     -- Superficie := Base * Altura; -- Asignación no compatible salvo que subtype Area is Distancia;
   --     -- Superficie := Area(Base * Altura);
   --     Superficie := Base * Altura; -- Ahora es válida después de declarar la función "*"
   --  end;
